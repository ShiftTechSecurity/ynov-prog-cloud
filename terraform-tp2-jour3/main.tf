...
resource "aws_ebs_volume" "encrypted_data" {
  ...
  tags = {
    Name        = "${local.resource_name_prefix}-ebs-${each.key}"
    Environment = var.environment
    Role        = each.value.role
  }
}

resource "aws_volume_attachment" "encrypted_data" {
  for_each = var.tiers

  device_name = "/dev/sdf"
  volume_id   = aws_ebs_volume.encrypted_data[each.key].id
...
resource "aws_kms_key" "ebs" {
  description             = "KMS key for encrypted EBS volumes"
  deletion_window_in_days = 7
  enable_key_rotation     = true

  tags = {
    Name        = "${local.resource_name_prefix}-ebs-kms"
    Environment = var.environment
  }
}

resource "aws_kms_alias" "ebs" {
  name          = "alias/${local.resource_name_prefix}-ebs"
